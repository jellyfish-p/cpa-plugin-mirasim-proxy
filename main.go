...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if err != nil {
		...
		writeResponse(response, errorEnvelope("plugin_error", err.Error(), status))
		return 1
	}
...
func cliproxyPluginShutdown() {
	_, _ = handlePluginMethod(pluginabi.MethodPluginShutdown, nil)
}
...
func callHost(method string, payload any) (json.RawMessage, error) {
	...
	if callCode != 0 {
		return nil, fmt.Errorf("host callback %s exited with code %d", method, int(callCode))
	}
	...
	if !env.OK {
		errMsg := "unknown host error"
		if env.Error != nil && env.Error.Message != "" {
			errMsg = env.Error.Message
		}
		return nil, fmt.Errorf("host callback error: %s", errMsg)
	}
